...
class BugJobMapViewSet(viewsets.ViewSet):
    ...
    def _get_bug_job_map_as_dict(bug_job_map):
        if bug_job_map.user:
            who = bug_job_map.user.email
        else:
            # assume bug is autoclassified if no user
            who = "autoclassify"

        return {
            'job_id': bug_job_map.job.project_specific_id,
            'bug_id': bug_job_map.bug_id,
            'type': bug_job_map.type,
    ...
    def list(self, request, project):
        ...
        jobs = Job.objects.filter(
            ...
            project_specific_id__in=job_ids).select_related('user')
        bug_job_maps = BugJobMap.objects.filter(job__in=jobs)
